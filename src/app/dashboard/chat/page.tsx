...
function AdminChatView() {
  ...
  // AI Assistant (AI Lead Matching) — same pinned entry admins get as agents.
  const openAssistant = () => {
    setActiveSession(null);
    setShowAssistant(true);
  };
  const closeAssistant = () => setShowAssistant(false);
  // Opening a conversation takes over the panel (mirrors the builder-network UX).
  ...
  return (
    <div className="h-[100dvh] lg:h-screen flex bg-[#FAF7F2] -mt-16 lg:mt-0">
      ...
      <div className={`w-full sm:w-80 lg:w-96 bg-white border-r border-[#E7E5E4] flex flex-col ${activeSession || showAssistant ? 'hidden sm:flex' : 'flex'}`}>
        <div className="px-3 py-2.5 sm:px-4 sm:py-3 bg-[#075E54]">
      ...
      {/* Right — Messages */}
      <div className={`flex-1 flex flex-col min-w-0 overflow-x-hidden ${activeSession || showAssistant ? 'flex' : 'hidden sm:flex'}`}>
        {showAssistant ? (
          <AiAssistantChat onBack={closeAssistant} />
        ) : activeSession ? (
          <>
